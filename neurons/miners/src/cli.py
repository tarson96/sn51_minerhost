...
def remove_executor_validator(address: str, port: int, validator: str):
    logger.info("Removing executor (%s:%d) that opens to validator(%s)", address, port, validator)
    executor_dao = ExecutorDao(session=next(get_db()))
    try:
        executor_dao.delete_by_address_port_validator(address, port, validator)
    except sqlalchemy.exc.IntegrityError as e:
        logger.error("Failed in removing an executor: %s", str(e))
    else:
        logger.info("Removed an executor(%s:%d) that opens to validator(%s)", address, port, validator)
